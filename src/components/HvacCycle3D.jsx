...
const FLUID_SEGMENTS = {
  discharge: {
    ...
    color: 0xef4444,
    text: 'Gaz chaud haute pression entre compresseur et condenseur. Tracé volontairement orthogonal pour lire le sens du cycle.',
    points: [
      [2.8, 0.95, -1.9],
      [3.55, 0.95, -1.9],
      [3.55, 1.55, -1.9],
  ...
  liquid: {
    ...
    state: 'Liquide HP',
    color: 0xfb923c,
    text: 'Liquide sous-refroidi vers le détendeur. Le chemin est placé en hauteur pour éviter les croisements avec l’aspiration.',
    points: [
      [3.2, 1.38, 1.75],
      [3.2, 1.38, 2.18],
      [-2.25, 1.38, 2.18],
  ...
  expansion: {
    ...
    color: 0x34d399,
    text: 'Mélange froid basse pression envoyé vers la batterie. Tronçon court pour bien repérer la détente.',
    points: [
      [-2.25, 0.75, -1.35],
      [-2.75, 0.75, -1.35],
      [-2.75, 0.75, -1.75],
  ...
  suction: {
    ...
    state: 'Vapeur BP',
    color: 0x38bdf8,
    text: 'Vapeur basse pression avec surchauffe de sécurité vers compresseur. Elle reste au niveau bas, séparée de la ligne liquide.',
    points: [
      [-3.15, 0.95, -1.75],
      [-3.15, 0.95, -2.45],
      [2.8, 0.95, -2.45],
  ...
  chilledSupply: {
    ...
    color: 0x67e8f9,
    text: 'Départ hydraulique vers les unités CRAH. Circuit placé côté gauche pour le distinguer du circuit frigorifique DX.',
    points: [
      [-1.95, 0.52, 1.95],
      [-3.35, 0.52, 1.95],
      [-3.35, 0.85, 1.95],
  ...
  chilledReturn: {
    ...
    color: 0x0284c7,
    text: 'Retour hydraulique après échange dans la batterie. Il emprunte une voie arrière distincte pour éviter les croisements.',
    points: [
      [-3.2, 0.78, 0.75],
      [-3.55, 0.78, 0.75],
      [-3.55, 0.78, 2.35],
  ...
  glycolHotToRoof: {
    label: 'Montée drycooler',
    layer: 'cooling',
    state: 'Glycol chaud vers toiture',
    color: 0xf97316,
    text: 'Montée du fluide caloporteur chaud depuis le local technique vers le drycooler en toit terrasse via le riser.',
    points: [
      [-1.05, 0.62, 1.95],
      [2.85, 0.62, 1.95],
      [2.85, 2.45, 1.95],
  ...
  glycolCoolFromRoof: {
    label: 'Descente drycooler',
    layer: 'cooling',
    state: 'Glycol refroidi depuis toiture',
    color: 0x22d3ee,
    text: 'Retour refroidi depuis le drycooler vers le local hydraulique. Descente séparée de la montée pour que le sens soit évident.',
    points: [
      [3.0, 1.72, 1.55],
      [3.0, 2.28, 1.55],
      [2.55, 2.28, 1.55],
...
function makeCurve(points) {
  ...
  path.userData = { points: vectors };
  return path;
}

function makeTube(points, color, radius = 0.045, options = {}) {
  const curve = makeCurve(points);
  const geometry = new THREE.TubeGeometry(curve, options.segments ?? 80, radius, options.radial ?? 14, false);
  const tubeMaterial = material(color, {
  ...
  // Coudes visuels aux changements de direction : lecture type plan technique,
  // sans courbes lissées et sans arrondir artificiellement le parcours.
  points.slice(1, -1).forEach(([x, y, z]) => {
    const elbow = new THREE.Mesh(
      new THREE.SphereGeometry(radius * 1.18, 16, 16),
      material(color, {
        metalness: options.metalness ?? 0.36,
        roughness: options.roughness ?? 0.3,
        opacity: options.opacity,
        emissive: options.emissive ?? color,
        emissiveIntensity: options.emissiveIntensity ?? 0.11
      })
    );
...
function makeElectricalCable(from, to, color) {
  ...
  return makeTube([
    from,
    [from[0], yLevel, from[2]],
    [to[0], yLevel, from[2]],
    [to[0], yLevel, to[2]],
    to
  ], color, 0.025, { opacity: 0.86, emissiveIntensity: 0.12, radial: 10, segments: 56 });
}
