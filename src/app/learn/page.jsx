...
import { BookOpen, ChevronLeft, ChevronRight, Gauge, Home, ListChecks, RotateCcw, ShieldCheck } from 'lucide-react';
...
function LearningList({ title, icon: Icon, items, tone = 'blue' }) {
  if (!items?.length) return null;

  const toneClasses = tone === 'emerald'
    ? 'border-emerald-500/20 bg-emerald-950/10 text-emerald-300'
    : 'border-blue-500/20 bg-blue-950/20 text-blue-300';
  const dotClass = tone === 'emerald' ? 'bg-emerald-300' : 'bg-blue-300';

  return (
    <div className={`mt-5 rounded-3xl border p-5 ${toneClasses}`}>
      ...
      <div className="flex min-h-screen flex-col lg:flex-row">
        <aside className="flex w-full flex-col border-r border-slate-800 bg-slate-950/80 lg:h-screen lg:w-[39rem]">
          ...
          <section className="flex-1 overflow-y-auto p-7">
            ...
            <LearningList title="À observer dans la pratique" icon={ListChecks} items={chapter.observe} />
            <LearningList title="Notes terrain" icon={ShieldCheck} items={chapter.field_notes} tone="emerald" />
        ...
        <section className="flex-1 overflow-y-auto p-5 lg:h-screen lg:p-8">
          <div className="mb-5 flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-400">Change de chapitre : les valeurs, le focus 3D, les états du fluide et le diagnostic se mettent à jour automatiquement.</p>
            </div>
          ...
          <div className="mt-5 grid gap-5 2xl:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="grid gap-5">
              <div className="rounded-3xl border border-blue-500/20 bg-blue-950/20 p-5">
                ...
                {activeSimulationParams.effect ? (
                  <p className="mt-3 text-sm leading-relaxed text-slate-300">{activeSimulationParams.effect}</p>
                ) : null}
